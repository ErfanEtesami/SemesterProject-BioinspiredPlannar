...
def run_simulation(flag_robot_model, flag_robot_state,
    ...
    if flag_record:
        record = pb.startStateLogging(pb.STATE_LOGGING_VIDEO_MP4, 'recording.mp4')
    ...
    for i in range(time_len):
        ...
        if spring_config == 1:
            ...
            toe_torque, knee_torque = torques_heel_tendon_new(-q_toe + 0.5, q_knee + 1, 0.02, 0.04, 0.04, 1000, 'flying', flag_block_toe)
        ...
        temp_motor_torques = np.matmul(np.transpose(jac), forces).reshape(2,)
        #  --- saturate the motor torques --- 
        temp_motor_torques = temp_motor_torques.clip(-torque_sat, torque_sat)

        # --- in case of control delay, save current command and apply it later --- 
        if delay:
            delayed_torques.append(temp_motor_torques)
            if len(delayed_torques) == delay + 1:
                motor_torques[i] = delayed_torques.pop(0)
            else:
                motor_torques[i] = [0., 0.]
        else:
            motor_torques[i] = temp_motor_torques
    ...
    f = open("logs.txt", 'a')
    f.write(f'Config: {spring_config}, height: {max_height:.2f}, Tsaved: {TsavedKnee:.3f}% / {TsavedTot:.3f}%, delay: {delay}\n')
...
def torques_heel_tendon_new(toe_angle, knee_angle, r_toe, r_knee1, r_knee2, k, mode, flag_block_toe):
